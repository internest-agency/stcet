...
export default function TNEACutoff() {
  ...
  useLayoutEffect(() => {
    const context = gsap.context(() => {
      ...
      const cards = sectionRef.current?.querySelectorAll(".cutoff-card");
          ...
          <div>
            ...
            <div className="grid border-y border-gray-200 sm:grid-cols-3">
              {cutoff.map((item, index) => (
                <div
                  key={item.subject}
                  className={`cutoff-card group p-6 sm:p-7 lg:p-8 ${
                    index !== 0 ? "border-t sm:border-l sm:border-t-0" : ""
                  } border-gray-200 transition-colors duration-300 hover:bg-primary-700`}
                >
                  <span className="text-xs font-bold text-gray-400 transition-colors group-hover:text-white/50">
                    {String(index + 1).padStart(2, "0")}
                  </span>

                  <span className="mt-8 block text-4xl font-black tracking-[-0.04em] text-primary-700 transition-colors group-hover:text-white sm:text-5xl">
                    {item.marks}
                  </span>

                  <span className="mt-2 block text-xs font-bold uppercase tracking-[0.15em] text-gray-500 transition-colors group-hover:text-white/70">
                    {item.subject}
                  </span>
                </div>
              ))}
