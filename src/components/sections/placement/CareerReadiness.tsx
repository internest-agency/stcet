  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      const cards = Array.from(
        section.querySelectorAll<HTMLElement>(".training-card"),
      );
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {trainingInitiatives.map((item) => (
              <div
                ...
                "
              >
                <span className="text-xs font-bold tracking-[0.16em] text-gray-400 transition-colors duration-300 group-hover:text-white/50">
                  {item.number}
                </span>

                <h3 className="mt-8 text-base font-extrabold leading-6 text-primary-700 transition-colors duration-300 group-hover:text-white sm:text-lg">
                  {item.title}
                </h3>

                <p className="mt-3 text-sm leading-6 text-gray-500 transition-colors duration-300 group-hover:text-white/70">
                  {item.description}
                </p>
